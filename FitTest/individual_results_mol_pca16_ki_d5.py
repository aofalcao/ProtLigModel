...
ids=[]
ki_uniprot_id_list = pickle.load(open('dist_5/ki_uniprot_id_list.pickle', 'rb'))
...
dict_id_mol_transformed = pickle.load(open(f'dict_mol_id_transformed_ki.pickle', 'rb'))
...
mol_ids = []
for i in ids_train:
    mol_tuples = pickle.load(open('/home/poliveira/individual_targets/ki_mol_fp/{}_with_id.pickle'.format(i),'rb'))
...
for it in range(10):
    print(it)
    model = RandomForestRegressor(n_estimators=200, n_jobs=20).fit(X_train, Y_train)
...
pickle.dump(results, open('ki/individual_results_mol_pca16_ki_d5.pickle', 'wb'))